...
def build_prompt(user_message: str, draft: str) -> str:
    ...
    return "\n".join(
        [
            ...
            'Always include "suggestions": 2-3 short tap-to-send replies the user might give YOU.',
            "They must directly answer or continue from YOUR message — never generic",
            'confirmations like "Yes, that sounds right" or "Just edit the draft"',
            "unless you actually proposed a draft change to confirm.",
            "Under ~8 words each. Example: if you ask \"What are you grateful for?\",",
            'suggestions might be ["My family\'s support","Feeling a little stronger","Quiet moments together"].',
